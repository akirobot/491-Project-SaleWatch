...
public class Game {
			...
			while (!done) {
				...
				if (inputMenuChoice == 1) {
					...
					if (players.get(playersTurn).getActionsTaken() == 0
						...
						System.out
							.println("88888 playersTurn is a " + playersTurn);
						playersTurn--;
						System.out
							.println("88888 playersTurn is b " + playersTurn);
	...
	public static void drawCardGameDeck(int player) {
		...
		if (cardDrawn.getCardType() == 3) {
			...
			System.out.println("006a Player Drew Group Card from gameDeck: "
				+ cardDrawn.getCardName());
	...
	public static void atkToControl(int player) {
		...
		while (!done) {
			...
			if (choice == 1) {
				// players.get(player).takeAction();
				System.out.println(
					"0010a atkToControlanother player is under construction");
				// players.get(player).takeAction();
			} else if (choice == 2) {
				...
				System.out
					.println("0010b TEST SAMPLE SCENARIO FOR ATK TO CONTROL");
				...
				if (choice == 1) {
					...
					if (players.get(player).getIlluminati().canAtkToControl()) { // Player's
						...
						System.out.println(
							"0010aab You don't have any available outgoing arrows for your Illuminati. So you can't attack to control with your Illuminati.");
					...
					if (players.get(player).getIlluminati()
						...
						System.out
							.println("00010cc You don't have any Group Cards");
					} else {
	...
	public static void atkToDestroy(int player) {
		...
		while (!done) {
			...
			if (choice == 1) {
				// players.get(player).takeAction();
				System.out.println(
					"0010a atkToDESTROYanother player is under construction");
				// players.get(player).takeAction();
			} else if (choice == 2) {
				...
				System.out
					.println("0010b TEST SAMPLE SCENARIO FOR ATK TO DESTROY");
	...
	private static void atkToDestroyUncontrolledAreaWithIlluminati(int player) {
		...
		System.out.println("\n012 Player must roll " + powerAndpowerDifference
	...
	private static void atkToDestroyUncontrolledAreaWithControlledGroupCard(
		...
		// ********************************************
		System.out.println(
			"0010aa Attack with your Group Card: " + attacker.getCardName());
	...
	private static void atkToControlUncontrolledAreaWithControlledGroupCard(
		...
		if (attacker.canAtkToControl()) {
			...
			if (successfulAttack) {
				...
				System.out
					.println("494 Select direction for your Group Card \n");
				int highestRange = 4;
				int lowestRange = 1;
				...
				while (!doneAdding) {
					...
					if (attacker.canAddToThisArrow(directionChoice)) {
						...
						System.out.println(
							"495 You can't add to an already occupied arrow. Or your Group Card does not have an outgoing arrow of that direction.");
					}
			...
			System.out.println(
				"00101aab You don't have any available outgoing arrows for your Group Card.");
	...
	private static void atkToControlUncontrolledAreaWithIlluminati(int player) {
		...
		if (successfulAttack) {
			...
			System.out
				.println("494 Select direction for your Illuminati Card \n");
			int highestRange = 4;
			int lowestRange = 1;
			boolean doneAdding = false;
			...
			while (!doneAdding) {
				...
				if (players.get(player).getIlluminati()
					...
					System.out.println(
						"495 You can't add to an already occupied arrow.");
				}
	...
	public static void createUnControlledArea() {
		...
		while (uncontrolledArea.size() != 4) {
			...
			if (cardDrawn.getCardType() == 3) {
				System.out.println("005 Drew Special Card from gameDeck: "
					+ cardDrawn.getCardName());
				// Don't put Special Card in uncontrolled Area
				// If this cards get drawn then put it back in the gameDeck
				...
				// Assume we drew Card type 2 here
				System.out.println("006 Drew Group Card from gameDeck: "
					+ cardDrawn.getCardName());
				// Keep drawing until there is four in the uncontrolled area
				// Add Group Card to uncontrolled Area
				uncontrolledArea.add((GroupCard) cardDrawn);
	...
	public static void createGameDeck() {
		...
		System.out.println("003 first card in groupCards array "
			+ groupCards.get(0).getCardName());
